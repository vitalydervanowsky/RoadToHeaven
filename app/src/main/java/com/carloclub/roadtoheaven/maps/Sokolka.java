...
//        Map.addQuestion(11, "Что из этого не является делом милосердия для тела ближнего?","Похоронить умершего","Посетить заключенного","Навестить больного","Посадить дерево",4,11);
        Map.quests = new MyMap.Quest[4];
        Map.addQuest(0);
        Map.quests[0].messageText="Начнём наш путь. Едь на заправку и заправь полный бак бензина.";
        Map.quests[0].messageIconMap="AZS";
        Map.quests[0].messageIconSource="icon_fuel";
        Map.quests[0].missionType="count_fuel";
        Map.quests[0].targetValue1=16000;
        Map.addQuest(1);
        Map.quests[1].messageText="Отлично! Теперь найди человека, которому нужна помощь. Ищи на карте такой рубин.";
        Map.quests[1].messageIconMap="";
        Map.quests[1].messageIconSource="icon_ruby";
        Map.quests[1].missionType="count_ruby";
        Map.quests[1].targetValue1=1;
        Map.addQuest(2);
        Map.quests[2].messageText="Отлично! Начнем собирать камни знаний. Найди на карте здания с такими камнями. ";
        Map.quests[2].messageIconMap="";
        Map.quests[2].messageIconSource="stones1";
        Map.quests[2].missionType="count_stones";
        Map.quests[2].targetValue1=1;
        Map.addQuest(3);
        Map.quests[3].messageText="Отлично! Теперь собери 7 камней или рубинов и построй из них разрушенный мост.";
        Map.quests[3].messageIconMap="icon_bridge";
        Map.quests[3].messageIconSource="stones1";
        Map.quests[3].missionType="count_stones";
        Map.quests[3].targetValue1=7;
...
//        Map.addQuestion(6, "Что из этого не является делом милосердия для тела ближнего?","Похоронить умершего","Посетить заключенного","Навестить больного","Посадить дерево",4,6);
        Map.quests = new MyMap.Quest[5];
        Map.addQuest(0);
        Map.quests[0].messageText="Вітаю цябе ў горадзе Сакулка! Тут мы пазнаёмімся з цудоўнымі “учынкаміміласэрнасці”.\nЖыхары гораду маюць камяні ведаў. Яны аддадуць іх за выкананне заданняў.\nЗ гэтых камянёў ты пабудуеш мост і зможаш працягнуць падарожжа. Вырушаем!";
        Map.quests[0].messageIconMap="AZS";
        Map.quests[0].messageIconSource="icon_fuel";
        Map.quests[0].missionType="count_fuel";
        Map.quests[0].targetValue1=1;
        Map.addQuest(1);
        Map.quests[1].messageText="Пачнем наша падарожжа. Едзь на запраўку и здабудзь поўны бак бензіна.";
        Map.quests[1].messageIconMap="AZS";
        Map.quests[1].messageIconSource="icon_fuel";
        Map.quests[1].missionType="count_fuel";
        Map.quests[1].targetValue1=16000;
        Map.addQuest(2);
        Map.quests[2].messageText="Выдатна! Цяпер накіроўвайся ў школу, каб здабыць неабходныя веды.";
        Map.quests[2].messageIconMap="";
        Map.quests[2].messageIconSource="icon_ruby";
        Map.quests[2].missionType="object_school";
        Map.quests[2].targetValue1=0;
        Map.addQuest(3);
        Map.quests[3].messageText="Выдатна! Цяпер знайдзі чалавека, якому патрэбна дапамога. Шукай на мапе такі рубін.";
        Map.quests[3].messageIconMap="";
        Map.quests[3].messageIconSource="icon_ruby";
        Map.quests[3].missionType="count_ruby";
        Map.quests[3].targetValue1=1;
        Map.addQuest(4);
        Map.quests[4].messageText="Выдатна! Цяпер збяры 7 такіх камянёў і пабудуй з іх разбураны мост.";
        Map.quests[4].messageIconMap="icon_bridge";
        Map.quests[4].messageIconSource="blue_stone";
        Map.quests[4].missionType="count_stones";
        Map.quests[4].targetValue1=7;
